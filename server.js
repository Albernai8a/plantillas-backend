...
// Generar número de programa (primer programa del día) - ZONA COLOMBIA
function generarNumeroProgramacion() {
  // Ajustar a zona horaria de Colombia (UTC-5)
  const fechaUTC = new Date();
  const fechaColombia = new Date(fechaUTC.getTime() - (5 * 60 * 60 * 1000));
  
  const year = fechaColombia.getFullYear().toString().slice(-2);
...
async function obtenerProgramaActivoDelDia() {
  try {
    // Usar fecha de Colombia (UTC-5)
    const fechaColombia = new Date(new Date().getTime() - (5 * 60 * 60 * 1000));
...
async function crearNuevoPrograma() {
  try {
    // Usar fecha de Colombia (UTC-5)
    const fechaColombia = new Date(new Date().getTime() - (5 * 60 * 60 * 1000));
    const hoy = fechaColombia.toISOString().split('T')[0];
...
app.post('/api/plantillas', async (req, res) => {
  try {
    ...
    const {
      ...
      numero_programa,
      numero_programacion,      // ← NUEVO: Campo que envía Lovable
      numeroProgramacion,       // ← NUEVO: Variante camelCase
      fecha_programacion,
      accion
    } = req.body;

    // ============================================
    // NORMALIZAR NÚMERO DE PROGRAMA
    // ============================================
    ...
    if (!accionDetectada) {
      ...
      } else if (plantillaExistente && numeroProgramaRecibido) {
    ...
    // CASO 2: Programar plantilla existente (CON NÚMERO RECIBIDO)
    if (plantillaExistente && accionDetectada === 'programar') {
      
      // ============================================
      // USAR NÚMERO RECIBIDO O PROGRAMA ACTIVO
      // ============================================
      let numeroProg;
      
      if (numeroProgramaRecibido) {
        // Usuario especificó un número (lo que envía Lovable)
        numeroProg = numeroProgramaRecibido;
        console.log('📅 Programando con número recibido:', numeroProg);
      } else {
        // Fallback: usar programa activo del día
        numeroProg = await obtenerProgramaActivoDelDia();
        console.log('📅 Programando con programa activo:', numeroProg);
      }
